  ...
  return (
    <div ref={wrapperRef}>
      ...
      >
        {defaultPrefix.emoji}
        {defaultPrefix.prefix}
        <ArrowDropDown className="root__arrow" />
      </button>
      {isOpen && formStatus !== FormStatus.disabled && (
        <div data-testid="list-dropdown" className="root__list">
          {allPrefixes.map((prefix) => {
            return <Item key={prefix.key} prefixItem={prefix} onClose={close} setPrefix={setPrefix} />;
          })}
        </div>
      )}
    </div>
